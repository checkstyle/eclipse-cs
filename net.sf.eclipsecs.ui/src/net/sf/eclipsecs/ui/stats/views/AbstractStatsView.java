...
public abstract class AbstractStatsView extends ViewPart {
    ...
    /**
     * Returns the view id of the concrete view. This is used to make separate filter settings
     * (stored in dialog settings) for different concrete views possible.
     *
     * @return the view id
     */
    ...
    /**
     * Callback for subclasses to refresh the content of their controls, since the statistics data
     * has been updated. <br/>
     * Note that the subclass should check if their controls have been disposed, since this method
     * is called by a job that might run even if the view has been closed.
     */
    ...
    /**
     * Causes the view to re-sync its contents with the workspace. Note that changes will be
     * scheduled in a background job, and may not take effect immediately.
     */
    ...
    private boolean updateNeeded(IResource[] oldResources, IResource[] newResources) {
        ...
        if (filter.enabled() && filter.onResource() != CheckstyleMarkerFilter.ON_ANY_RESOURCE
