 ...
 * URL syntax.
 */
public class CheckstyleProjectConfigurator extends AbstractMavenPluginProjectConfigurator {
        ...
        throws CoreException
    {
        try {
            ...
            ICheckConfiguration checkConfig = createOrUpdateLocalCheckConfiguration(project,
                    workingCopy, ruleSet);
            if (checkConfig == null) {
                return;
            }
    ...
    private ICheckConfiguration createOrUpdateLocalCheckConfiguration(IProject project,
            ProjectConfigurationWorkingCopy projectConfig, URL ruleSet)
            ...
            for (CheckConfigurationWorkingCopy copy : workingCopies) {
                if (CONFIGURATION_NAME.equals(copy.getName())) {
                    if (remoteConfigurationType.equals(copy.getType())) {
                        console
                                .logMessage("A local Checkstyle configuration allready exists with name "
                                        + CONFIGURATION_NAME
                        ...
                        console
                                .logError("A local Checkstyle configuration allready exists with name "
    ...
    private Properties extractCustomProperties(Plugin plugin, ClassLoader pluginClassloader) {
        ...
        if (propertyExpansion != null) {
            try {
                ...
                console.logError("Failed to parser checkstyle propertyExpansion "
                        + propertyExpansion);
            }
        }
    ...
    /**
     * Extract the configured supression filters from the checkstyle
     * configuration
     * 
     * @see http://maven.apache.org/plugins/maven-checkstyle-plugin/examples/
     *      suppressions-filter.html
     */
    ...
    private Plugin getCheckstylePlugin(MavenProject mavenProject) {
        return mavenProject.getPlugin(CHECKSTYLE_PLUGIN_GROUPID + ":"
                + CHECKSTYLE_PLUGIN_ARTIFACTID);
