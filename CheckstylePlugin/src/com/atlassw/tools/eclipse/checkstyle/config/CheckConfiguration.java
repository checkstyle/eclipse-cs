...
public class CheckConfiguration implements Cloneable, XMLTags, Configuration, Comparable
{
    ...
    /**
     * Construct from a standard Checkstyle config file DOM node.
     *
     * @param node     The node.
     *
     * @param name  The <code>CheckConfiguration</code>'s name.
     *
     * @param defaultCSFormat    if ture parse in default checkstyle format, if false parse using
     *                          plug-in enhanced format.
     *
     * @throws CheckstyleException   Could not properly construct the
     *                                <code>CheckConfiguration</code> from the XML node.
     */
    public CheckConfiguration(Node node, String name, boolean defaultCSFormat)
        throws CheckstylePluginException
    {
        if (defaultCSFormat)
        {
			...
			{
				String msg = "Checkstyle rule missing name";
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (child.getNodeName().equals(MODULE_TAG))
                {
                    RuleConfiguration rule = null;

                    try
                    {
                        rule = new RuleConfiguration(child, true);
                    }
                    catch (CheckstylePluginException e)
                    {
                        rule = null;
