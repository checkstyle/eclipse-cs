 ...
 *
 */
public class DefaultProjectConfiguration implements Cloneable, ProjectConfiguration {

  /** The project. */
  private IProject project;

  /** The local check configurations. */
  private List<CheckConfiguration> localCheckConfigs;
   ...
   *          settings
   */
  public DefaultProjectConfiguration(IProject project, List<CheckConfiguration> localConfigs,
          List<FileSet> fileSets, List<AuditFilter> filters, boolean useSimpleConfig,
          boolean synchFormatter) {
    this.project = project;
    localCheckConfigs = localConfigs != null ? Collections.unmodifiableList(localConfigs)
            : Collections.unmodifiableList(new ArrayList<CheckConfiguration>());
    ...
    if (filters != null) {
      ...
      for (int i = 0, size = this.filters.size(); i < size; i++) {
        ...
        AuditFilter standardFilter = this.filters.get(i);

        for (int j = 0, size2 = filters.size(); j < size2; j++) {
          AuditFilter configuredFilter = filters.get(j);
  ...
  @Override
  public DefaultProjectConfiguration clone() {
    DefaultProjectConfiguration clone = null;
    try {
      clone = (DefaultProjectConfiguration) super.clone();
  ...
  public boolean equals(Object obj) {
    ...
    if (obj == null || !(obj instanceof DefaultProjectConfiguration)) {
      return false;
    }
    if (this == obj) {
      return true;
    }
    DefaultProjectConfiguration rhs = (DefaultProjectConfiguration) obj;
