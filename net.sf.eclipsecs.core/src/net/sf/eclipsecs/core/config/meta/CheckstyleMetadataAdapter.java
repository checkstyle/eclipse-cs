...
public class CheckstyleMetadataAdapter {
    ...
    /**
     * Mapping for all module property datatype acquired from checkstyle metadata to the internal
     * property impletation.
     */
        ...
        for (ModuleDetails moduleDetails : allModuleDetails) {
            ...
            if (moduleClassName.startsWith("com.puppycrawl.tools.checkstyle")) {
                final String[] packageTokens =
                    moduleDetails.getFullQualifiedName().split(DOT_PATTERN);
                group = groups.get(packageToGroupName.get(packageTokens[packageTokens.length - 2]));
            }
    ...
    /**
     * Create metadata for modules not present in the previously eclipse provided metadata. Work in
     * progress.
     *
     * @param group
     *            the rule group
     * @param moduleDetails
     *            module details fetched from checkstyle metadata
    ...
    /**
     * Create module property config data based on current/default data, which are overridden
     * partially with all the metadata fetched from checkstyle.
     *
     * @param moduleDetails
     *            checkstyle metadata of the parent module of the property
     * @param modulePropertyDetails
     *            checkstyle metadata of the property
    ...
    private static List<String> getEnumValues(String className) {
        ...
        try {
            providerClass =
                CheckstylePlugin.getDefault().getAddonExtensionClassLoader().loadClass(className);
            @SuppressWarnings({
                "rawtypes", "unchecked"
            })
